...
main(int argc, char **argv)
{
        ...
        // call aprun
        if ((aprunPid = launchAprun_barrier(&argv[1],0,0,0,0,NULL,NULL)) <= 0)
        {
        ...
        if ((app_nodes = getNumAppNodes(aprunPid)) == 0)
        {
                fprintf(stderr, "getNumAppNodes failed.\n");
                killAprun(aprunPid, 9);
                return 1;
        }
        // get number of PEs in app
        if ((num_pes = getNumAppPEs(aprunPid)) == 0)
        {
        ...
        if (sendCNodeExec(aprunPid, LAUNCHER, tool_argv, (char **)NULL, 0))
        {
	...
	(void)getchar();
        ...
        // release barrier
        if (releaseAprun_barrier(aprunPid))
        {
