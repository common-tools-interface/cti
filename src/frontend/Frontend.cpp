...
// FIXME: Remove this during integration.
cti::overwatch_handle
Frontend::make_overwatch_handle(pid_t targetPid) {
    if (targetPid) {
        // parent case
        if (targetPid < 0) {
            throw std::runtime_error("fork failed");
        }
        return cti::overwatch_handle{getOverwatchPath(), targetPid};
    }
    else {
        // child case
        return cti::overwatch_handle{};
    }
}
...
: m_stage_deps{true}
{
    ...
    m_overwatch_path = cti::accessiblePath(m_base_dir + "/libexec/" + CTI_OVERWATCH_BINARY);
    m_dlaunch_path = cti::accessiblePath(m_base_dir + "/libexec/" + CTI_DLAUNCH_BINARY);
}
