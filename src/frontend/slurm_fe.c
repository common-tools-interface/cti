...
cti_mpir_procTable_t *			_cti_slurm_getPids(pid_t srunPid);
...
cti_slurm_registerJobStep(pid_t launcher_pid)
{
	...
	if ((sinfo->layout = _cti_cray_slurm_getLayout(app_info->jobid, app_info->stepid)) == NULL)
	{
		// error already set
		_cti_slurm_consumeSlurmInfo(sinfo);
		return 0;
...
_cti_slurm_getPids(pid_t srunPid)
{
	...
	cti_mpir_procTable_t *	pids; // return object
...
_cti_slurm_getHostName(void)
{
	...
	if ((nid_fp = fopen(ALPS_XT_NID, "r")) != NULL)
	{
	    ...
	    {
		    fprintf(stderr, "_cti_slurm_getHostName fgets failed.\n");
		    fclose(nid_fp);
		    return NULL;
	    }
        ...
        if ((errno == ERANGE && nid == INT_MAX)
                || (errno != 0 && nid == 0))
        {
            fprintf(stderr, "_cti_slurm_getHostName: strtol failed.\n");
            return NULL;
        }
        ...
        if (eptr == file_buf)
        {
            fprintf(stderr, "_cti_slurm_getHostName: Bad data in %s\n", ALPS_XT_NID);
            return NULL;
        }
	    ...
	    {
		    fprintf(stderr, "_cti_slurm_getHostName asprintf failed.\n");
            free(hostname);
            hostname = NULL;
		    return NULL;
	    }
    ...
    else // Fallback to standard hostname
    {
	    ...
	    {
		    fprintf(stderr, "_cti_slurm_getHostName: malloc failed.\n");
		    return NULL;
	    }

        if (gethostname(hostname, HOST_NAME_MAX) < 0)
        {
            fprintf(stderr, "%s", "_cti_slurm_getHostName: gethostname() failed!\n");
            hostname = NULL;
		    return NULL;
        }
    }
